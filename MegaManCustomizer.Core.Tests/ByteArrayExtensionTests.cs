...
public class ByteArrayExtensionTests
{
    ...
    [InlineData(new byte[] { 0xFF, 0xFF, 0 }, 0, 1, "255.99609375")]
    [InlineData(new byte[] { 1, 0x80, 0 }, 0, 1, "1.5")]
    public void SetDecimal_SetsTwoBytes(byte[] bytes, int wholeAddress, int fractionAddress, string valueText)
    {
        var value = decimal.Parse(valueText);
        (byte whole, byte fraction) pair = value.ToBytePair();
        Assert.Equal(pair, bytes.SetDecimal(wholeAddress, fractionAddress, value));
        Assert.Equal(pair.whole, bytes[wholeAddress]);
        Assert.Equal(pair.fraction, bytes[fractionAddress]);
    }
}
